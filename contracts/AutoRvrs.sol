...
contract AutoRvrs is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 reverseAtLastUserAction; // keeps track of crystl deposited at the last user action
    ...
    uint256 public constant MAX_WITHDRAW_FEE_PERIOD = 80 hours; // 3 days
    ...
    function deposit(uint256 _amount, address _to) external whenNotPaused {
        ...
        user.reverseAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Reinvests RVRS tokens into masterchef
     * @dev Only possible when contract not paused.
     */
    function earn() external notContract whenNotPaused {
        IMasterchef(masterchef).harvest(0, address(this));
    ...
    function calculateHarvestCrystalRewards() external view returns (uint256) {
        uint256 amount = IMasterchef(masterchef).pendingReward(0, address(this));
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IMasterchef(masterchef).withdraw(0, balWithdraw, address(this));
        ...
        if (user.shares > 0) {
            ...
            user.reverseAtLastUserAction = 0;
        }
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IMasterchef(masterchef).deposit(0, bal, address(this), msg.sender);
