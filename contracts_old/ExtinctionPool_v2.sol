...
contract BondingPool is Ownable {
    ...
    struct UserInfo 
    {
        uint256 amount;                     // How many BondToken the user has provided.
    ...
    mapping (address => UserInfo) public userInfo;     // Info of each user that stakes BondToken tokens.

    event Withdraw(address indexed user, uint256 amount);
    event Deposit(address indexed user, uint256 amount);
    event Burn(uint256 amount);
    event ReverseumDeposit(uint256 amount);
    ...
    constructor(
        ...
        require(_burnRate < BURN_RATE_DIVISOR, "_burn rate too high!");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > poolInfo.lastRewardBlock && stakedBondTokens != 0) {
            uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
            uint256 tokenReward = multiplier.mul(rewardPerBlock);
            accRewardPerShare = accRewardPerShare.add(tokenReward.mul(1e12).div(stakedBondTokens));
        }
    ...
    function updatePool() public {
        ...
        if (stakedBondTokens == 0) {
            poolInfo.lastRewardBlock = block.number;
            return;
        }
        uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(rewardPerBlock);
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(stakedBondTokens));
        poolInfo.lastRewardBlock = block.number;
    }
    ...
    function transact(uint256 _amount, address _to) public {
        ...
        if (block.number < lockBlock && _amount != 0) {
            // First burn and send lp tokens to reverseum
            BondToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (burnRate > 0) {
                BondToken.safeTransfer(burnAddress, _amount.mul(burnRate).div(BURN_RATE_DIVISOR));
                emit Burn(_amount.mul(burnRate).div(BURN_RATE_DIVISOR));
            }
            uint256 remainingAmount = BondToken.balanceOf(address(this));
            BondToken.safeTransfer(reverseum, remainingAmount);
            emit ReverseumDeposit(user.amount);
    ...
    function activateEmergency(address _to) external onlyOwner returns(bool) {
        ...
        // TODO - require ~3 week offset after end date 604800
        emergencyActive = true;
        if (BondToken == REWARD) {
            return REWARD.transfer(_to, REWARD.balanceOf(address(this)) - stakedBondTokens);
        } else {
    ...
    function emergencyWithdraw() external {
        ...
        BondToken.safeTransfer(address(msg.sender), user.amount);
        stakedBondTokens = stakedBondTokens.sub(user.amount);
